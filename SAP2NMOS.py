...
def rebuild_sender_key_cache():
    global SENDER_CACHE, SENDER_KEY_CACHE, CACHE_TS

    print("[CACHE] rebuild sender key cache")

    SENDER_CACHE = fetch_all_senders()
    SENDER_KEY_CACHE = {}

    for s in SENDER_CACHE:
        sid = s.get("id")
        manifest_url = s.get("manifest_href")

        if not sid or not manifest_url:
            continue
        ...
        try:
            ...
            parsed = parse_sdp(r.text)
            key = build_match_key(parsed)

            if key:
                SENDER_KEY_CACHE[key] = s
                print(f"[CACHE] {sid} -> {key}")

        except Exception as e:
            print("[CACHE] manifest error", e)
...
def get_existing_senders():
    ...
    if now - CACHE_TS > CACHE_TTL:
        rebuild_sender_key_cache()
    else:
        print("[CACHE] using cached sender keys")
...
def find_existing_sender(sdp):
    ...
    sender_map = get_existing_senders()

    s = sender_map.get(key)
...
def build_connection_active(sender_id):
    s = SENDERS.get(sender_id)
    if not s:
        return None

    # SDP parsen für Transportdaten
    stream = next((x for x in STREAMS.values() if x["sender_id"] == sender_id), None)
    if not stream:
        return None

    p = parse_sdp(stream["sdp"])
    ...
    return {
        ...
        "receiver_id": None,
        "transport_params": [{
            "destination_port": p.get("port", 5004),
            "source_port": p.get("port", 5004),
            "source_ip": p.get("src_ip", get_ip()),
            "destination_ip": p.get("ip"),
            "rtp_enabled": True
        }]
    }
...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if path == "/x-nmos/connection/v1.1/single/senders":
            return self.send_json(list(SENDERS.keys()))

        # Sender spezifisch
        for sid in SENDERS.keys():

            # /active
            if path == f"/x-nmos/connection/v1.1/single/senders/{sid}/active":
                data = build_connection_active(sid)
                if data:
                    return self.send_json(data)

            # /transportfile (SDP)
            if path == f"/x-nmos/connection/v1.1/single/senders/{sid}/transportfile":
                stream = next((x for x in STREAMS.values() if x["sender_id"] == sid), None)
                if stream:
                    self.send_response(200)
                    self.send_header("Content-Type", "application/sdp")
                    self.end_headers()
                    self.wfile.write(stream["sdp"].encode())
                    return
